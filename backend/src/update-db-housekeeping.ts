...
async function main() {
  console.log('Menyiapkan pembaruan database untuk Modul Housekeeping...');

  try {
    // 1. Ubah tipe ENUM pada kolom status_kamar
    console.log('1. Memperbarui tipe ENUM status_kamar...');
    ...
    console.log('2. Membuat tabel tugas_housekeeping...');
    await prisma.$executeRawUnsafe(`
    ...
    console.log('3. Memperbarui Trigger trg_after_checkout_insert...');
    await prisma.$executeRawUnsafe(`DROP TRIGGER IF EXISTS trg_after_checkout_insert`);
    ...
    console.log('Pembaruan database sukses!');
  } catch (error) {
